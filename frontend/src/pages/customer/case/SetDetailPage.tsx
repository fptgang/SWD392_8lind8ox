      ...
      <div className="flex items-center">
        ...
        <div className="ml-auto">
          <Progress 
            ...
            status={availablePercentage > 50 ? "active" : availablePercentage > 0 ? "normal" : "exception"}
    ...
    <div className="relative">
      ...
      >
        {sortedSlots.map((slot, index) => (
          <div 
            ...
            style={{ minWidth: '200px', width: 'calc(100% / 6)', maxWidth: '180px' }}
          >
            <motion.div
              whileHover={{ 
                y: [0, -15, 0], 
              ...
              onClick={() => slot.state === SlotDtoStateEnum.Available && handleSlotClick(slot)}
              className="h-full"
            >
              <Badge 
                count={index + 1} 
                ...
                >
                  <div className="absolute inset-0 flex items-center justify-center">
                    {slot.state === SlotDtoStateEnum.Available ? (
                      <motion.div animate={{ rotate: [0, 5, 0, -5, 0] }} transition={{ repeat: hoveredSlotId === slot.slotId ? Infinity : 0, duration: 0.5 }}>
                        <Image
                          src={set.sku?.image?.imageUrl || 'https://via.placeholder.com/100x100?text=?'}
                          alt={`Slot ${index + 1}`}
                          preview={false}
                      ...
                      <Image
                        ...
                        alt={`Slot ${index + 1}`}
                        preview={false}
                        style={{ maxHeight: 90, maxWidth: '100%', objectFit: 'contain', opacity: 0.5 }}
                      />
                    )}
                  </div>
      ...
      >
        ...
        >
          {relatedToysLoading ? (
              ...
              {relatedToysData.data.map((toy) => {
                ...
                return (
                  <Col key={toy.toyId} xs={12} sm={8} md={6} lg={4}>
                    <Tooltip title={hasVideoProof ? "Click to see proof" : "No video proof available"}>
                      <motion.div
                        whileHover={{ scale: 1.05, y: -5 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => hasVideoProof && handleToyClick(toy, toySlot)}
                      >
                        <Card
                          ...
                          <Card.Meta
                            ...
                            description={
                              <div className="text-center">
                                <Tag color={toy.rarity === 'SECRET' ? 'gold' : 'blue'} className="mt-2">
                                  {toy.rarity || 'REGULAR'}
                                </Tag>
                              </div>
                            }
            ...
            <div className="py-16 text-center">
              <GiftOutlined style={{ fontSize: 48 }} className="text-gray-300 mb-4" />
              <Title level={4} className="text-gray-500">No toys discovered yet</Title>
              <Text type="secondary">Start opening mystery boxes to discover toys!</Text>
            </div>
          )}
        </Card>
      </motion.div>
        ...
        <Col xs={24} md={10}>
          ...
          >
            <Card className="mb-6 shadow-md overflow-hidden">
              <Image
                src={set.sku?.image?.imageUrl || 'https://via.placeholder.com/600x400?text=No+Image'}
                alt={set.sku?.name}
            ...
            <Card className="mb-6 shadow-md">
              <Space direction="vertical" size="large" className="w-full">
                ...
                <Descriptions title="Product Details" column={1} bordered>
                  <Descriptions.Item label={<><BoxPlotOutlined /> SKU ID</>}>
                    {set.sku?.skuId}
                  </Descriptions.Item>
                  <Descriptions.Item label={<><InfoCircleOutlined /> Description</>}>
                    {set.sku?.name || 'No description available'}
                  </Descriptions.Item>
                  <Descriptions.Item label={<><GiftOutlined /> Total Slots</>}>
                    {set.slots?.length || 0} slots available
                  </Descriptions.Item>
                  <Descriptions.Item label={<><ClockCircleOutlined /> Created</>}>
              ...
              >
                <Descriptions column={1}>
                  ...
                  <Descriptions.Item label="Description">
                    {set.blindBox.brand.description || 'No description available'}
                  </Descriptions.Item>
                </Descriptions>
              </Card>
            </motion.div>
      ...
      <Modal
        ...
        footer={[
          ...
          <motion.div key="add" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Button 
              type="primary" 
          ...
          {selectedSlot && (
            <Row gutter={[16, 16]} align="middle">
              <Col span={12} className="text-center">
                ...
                >
                  <Image
                    src={set.sku?.image?.imageUrl || 'https://via.placeholder.com/200x200?text=?'}
              ...
              <Col span={12}>
                <Descriptions title="Selected Box" column={1} size="small" bordered>
      ...
      <Modal
        ...
        footer={[
          <Button key="close" type="primary" onClick={() => setIsVideoProofModalOpen(false)}>
            Close
          </Button>
        ]}
        width={800}
