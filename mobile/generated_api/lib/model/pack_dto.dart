...
class PackDto {
  /// Returns a new [PackDto] instance.
  PackDto({
    this.packId,
    this.brandId,
    this.name,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PackDto &&
    other.packId == packId &&
  ...
  @override
  String toString() => 'PackDto[packId=$packId, brandId=$brandId, name=$name, description=$description, currentPrice=$currentPrice, promotionalCampaignId=$promotionalCampaignId, isVisible=$isVisible, createdAt=$createdAt, updatedAt=$updatedAt, images=$images, guaranteedToys=$guaranteedToys, toyCount=$toyCount]';
  ...
  /// Returns a new [PackDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PackDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PackDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PackDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PackDto> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PackDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PackDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PackDto> mapFromJson(dynamic json) {
    final map = <String, PackDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PackDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PackDto-objects as value to a dart map
  static Map<String, List<PackDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PackDto>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PackDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
