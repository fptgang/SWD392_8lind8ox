...
class VoucherRepositoryImpl implements VoucherRepository {
  ...
  @override
  Future<PaginationResponseGeneric<VoucherModel>> getVouchers(
      Pageable pageable, String filter, String search) async {
    try {
      GetVouchers200Response? response = await _apiService.getVouchers(
          pageable: pageable, filter: filter, search: search);
      if (response == null) {
        throw Exception('Cannot get voucher information');
      }
      PaginationResponseGeneric<VoucherModel>? voucherModels =
          PaginationResponseMapper.toModel(
              dto: response, fromDTO: (data) => VoucherMapper.toModel(data));
