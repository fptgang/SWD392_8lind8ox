...
class ShippingInfoBloc extends Bloc<ShippingInfoEvent, ShippingInfoState> {
  ...
  ShippingInfoBloc(this._shippingInfoRepository)
      : _paginationState =
            ShippingInfoPaginationState(pageable: Pageable(page: 1, size: 20)),
  ...
  Future<void> _onGetShippingInfos(
    ...
    try {
      ...
      if (isLastPage) {
        ...
        pagingController.appendPage(shippingInfos.content, event.pageKey + 1);
      }
      ...
      _dataState =
          _dataState.copyWith(shippingInfoResponseModel: shippingInfos);
      emit(_dataState);
    } catch (error) {
  ...
  Future<void> _onCreateShippingInfo(
    ...
    try {
      final createdShippingInfo =
          await _shippingInfoRepository.createShippingInfo(
      ...
      debugPrint(
          'token from shipping info bloc: ${Hive.box('authentication').get('loginToken')}');
