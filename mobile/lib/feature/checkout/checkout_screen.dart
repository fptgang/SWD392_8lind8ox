...
class _CheckoutScreenState extends State<CheckoutScreen> {
  // Only using VNPAY as the payment method
  ...
  final VoucherRepository _voucherRepository = getIt<VoucherRepository>();
  ...
  Future<void> onSelectVoucher(voucher.VoucherDto voucherDto) async {
    try {
      if (voucherDto.voucherId != null) {
        final voucher =
            await _voucherRepository.getVoucherById(voucherDto.voucherId!);
  ...
  Future<List<voucher.VoucherDto>> _loadVouchers() async {
    try {
      ...
      return vouchersResponse.content
          .map((v) => voucher.VoucherDto(
  ...
  void _showVoucherSelectionDialog() async {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _buildVoucherSelectionDialog(vouchers),
    );
  }
  ...
  // Navigate to edit cart
  void _navigateToEditCart() {
    context.pop(); // Navigate back to cart
  }
  ...
  Future<void> placeOrder() async {
    ...
    try {
      // Use CartGlobalBloc to place order with VNPAY only
      _cartBloc.add(PlaceOrder(paymentMethod: 'VNPAY'));
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          OrderItemsSection(
            items: displayItems,
            onEditCart: _navigateToEditCart,
          ),
          ...
          voucher.VoucherSection(
            selectedVoucher: cartState.voucher != null
                ...
                : null,
            onSelectVoucher: (_) => _showVoucherSelectionDialog(),
          ...
          const SizedBox(height: 16),
          payment.PaymentMethodSection(
            selectedPaymentMethod: payment.CartDtoPaymentMethodEnum.VNPAY,
            onSelectPaymentMethod: (_) {}, // No-op as only VNPAY is supported
                ...
                children: [
                  ...
                  payment.PaymentMethodSection(
                    selectedPaymentMethod:
                        payment.CartDtoPaymentMethodEnum.VNPAY,
                    onSelectPaymentMethod:
                        (_) {}, // No-op as only VNPAY is supported
                    finalTotal: cartState.finalTotal,
                  ),
                ],
              ),
        ...
        children: [
          ...
          Expanded(
            child: vouchers.isEmpty
                ? _buildEmptyVouchersList(context)
                : ListView.separated(
                    padding: const EdgeInsets.all(16),
