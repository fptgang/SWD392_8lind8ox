...
class SearchResults extends StatelessWidget {
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  if (box.brand?.name != null && box.brand!.name!.isNotEmpty) ...[
                  ...
                  if (box.skus != null && box.skus!.isNotEmpty && box.skus!.first.price != null) ...[
                    const SizedBox(height: 8),
                    Text(
  ...
  Widget _buildSpecsRow(BuildContext context, BlindBoxModel box) {
    ...
    final materialMatch = RegExp(r'Material:\s*([\w\/]+)').firstMatch(description);
    if (materialMatch != null) {
      specs.add('Material: ${materialMatch.group(1)!}');
    }

    final heightMatch = RegExp(r'Height:\s*([\d\.\-\s]+inches)').firstMatch(description);
    if (heightMatch != null) {
      specs.add('Height: ${heightMatch.group(1)!}');
    }

    final releaseDateMatch = RegExp(r'Release Date:\s*([\w\s,\.]+\d{4})').firstMatch(description);
    ...
    return specs.isNotEmpty
        ? Wrap(
      spacing: 16,
      children: specs.map((spec) => _buildSpecChip(context, spec)).toList(),
    )
  ...
  Widget _buildItemImage(BlindBoxModel box) {
    ...
    if (box.description != null && box.description!.contains('<img')) {
      final imgMatch = RegExp(r'<img src="([^"]+)"').firstMatch(box.description!);
      if (imgMatch != null) {
